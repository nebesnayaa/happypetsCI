...
class Order extends CI_Controller
{
	...
	{
		$data["page_title"] = "Complete Order Details";
		$data["banks"] = $this->Bank_model->getAllBanks();

		$this->form_validation->set_rules('receipent_name', 'Recipient\'s Name', 'required');
		$this->form_validation->set_rules('receipent_phone', 'No Hp Penerima', 'required');
		$this->form_validation->set_rules("receipent_address", "Address", "required");

		if (empty($_FILES['payment_slip']['name'])) {
			$this->form_validation->set_rules('payment_slip', 'Payment Receipt', 'required');
		}
