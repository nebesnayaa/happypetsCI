...
class Package extends CI_Controller
{
	...
	{
		$data["page_title"] = "Manage Package grooming";
	...
	public function ajaxList()
	{
		...
		foreach ($list as $package) {
			...
			$row[] = "UAH " . number_format($package->cost_for_cat);
			$row[] = "UAH " . number_format($package->cost_for_dog);
			...
			$row[] = '<a class="btn btn-icon btn-warning" href="javascript:void(0)" title="Edit" onclick="editPackage(' . "'" . $package->package_id . 			"'" . ')"><i class="fas fa-edit"></i></a>
				  <a class="btn btn-icon btn-danger" href="javascript:void(0)" title="delete" onclick="deletePackage(' . "'" . $package->package_id . "'" . ')"><i class="fas fa-trash"></i></a>';
	...
	private function _validate()
	{
		...
		if ($this->input->post('name') == '') {
			...
			$data['error_string'][] = 'Name paket harus diisi';
			$data['status'] = FALSE;
		}

		if ($this->input->post('cost_for_cat') == '') {
			$data['inputerror'][] = 'cost_for_cat';
			$data['error_string'][] = 'Cost Grooming harus diisi';
			$data['status'] = FALSE;
		}
