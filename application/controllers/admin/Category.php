...
class Category extends CI_Controller
{
	...
	public function ajaxList()
	{
		...
		foreach ($list as $category) {
			...
			$row[] = '<a class="btn btn-icon btn-warning" href="javascript:void(0)" title="Edit" onclick="editCategory(' . "'" . $category->category_id . 			"'" . ')"><i class="fas fa-edit"></i></a>
                  <a class="btn btn-icon btn-danger" href="javascript:void(0)" title="delete" onclick="deleteCategory(' . "'" . $category->category_id . "'" . ')"><i class="fas fa-trash"></i></a>';
	...
	private function _validate()
	{
		...
		if ($this->input->post("name") == "") {
			$data['error_string'] = 'Name category harus di isi';
		...
		// 	$data['inputerror'][] = 'image';
		// 	$data['error_string'][] = 'Picture harus di upload';
		// 	$data['status'][] = FALSE;
		// }
