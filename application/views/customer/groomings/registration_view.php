							...
							<div class="row">
								<div class="col-8 mx-auto">
									<form action="<?= base_url("grooming/register") ?>" method="post">
										...
										<div class="form-group">
											<label for="customer_address">Address Customer</label>
										...
										<div class="form-group d-none" id="catPackage">
											<label for="package_id">Package grooming Cat</label>
											<select name="package_id" id="package_id" class="form-control">
												<option value="" disabled selected>--Choose Package grooming--</option>
												<?php foreach ($packages as $package) : ?>
													<option value="<?= $package["package_id"] ?>"><?= $package["name"] ?> | UAH <?= number_format($package["cost_for_cat"]) ?></option>
												<?php endforeach; ?>
											</select>
										</div>
										<div class="form-group d-none" id="dogPackage">
											<label for="package_id">Package grooming Dog</label>
											<select name="package_id" id="package_id" class="form-control">
												<option value="" disabled selected>--Choose Package grooming--</option>
												<?php foreach ($packages as $package) : ?>
													<option value="<?= $package["package_id"] ?>"><?= $package["name"] ?> | UAH <?= number_format($package["cost_for_dog"]) ?></option>
												<?php endforeach; ?>
											</select>
										</div>
										<div class="form-group">
											<label for="notes">Comment (Optional)</label>
